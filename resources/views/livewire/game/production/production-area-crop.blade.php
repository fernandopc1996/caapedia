<div class="text-gray-900">
    @if ($production)
    <x-layout.header title="{{ $production->name }}" />
        <livewire:game.production.production-crop :production="$production"/>
        @if (isset($production) && $production->playerProducts->whereNotNull('player_action_id')->count() >= 1)
        ...
        @endif
        @if (isset($production) && $production->playerProducts->whereNull('player_action_id')->count() >= 1)
            <h3 class="mt-6 text-2xl font-semibold text-gray-900 mb-2">Encontrado por último durante manejo</h3>
            <div>
                <livewire:game.production.production-produced :production="$production" :manejo="true"/>
            </div>
        @endif
    @else
                    ...
                    {{ $method->available ? '' : 'opacity-50 pointer-events-none' }}"
                        ...
                        <div class="grid grid-cols-2 gap-2 text-sm mb-3">
                            <x-game.production.build-item icon="fas.clock" label="Tempo"
                                value="{{ $method->execution['time'] }}h" color="gray-600" />
                            ...
                            <x-game.production.build-item icon="fas.object-group" label="Área"
                                value="{{ $method->execution['area'] }} ha" color="green-600" />
                            <x-game.production.build-item icon="fas.tint" label="Água"
                                value="{{ $method->execution['water'] }} m³" color="blue-600" />
                        </div>

                        @if ($method->available)
                            <x-selects.character-selector-params :characters="$availableCharacters" wire:model="selectedCharacter"
                                :params="['native_cleaning_coid' => $method->id]" button-label="Preparar área" />
                        @else
